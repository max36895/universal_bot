...
class YandexSpeechKit extends YandexRequest
{
     ...
     * @var string|null $text Текст, который нужно озвучить, в кодировке UTF-8.
     ...
     * @var string $voice Желаемый голос для синтеза речи из списка. Значение параметра по умолчанию: oksana.
     */
    public $voice;
    /**
     * Эмоциональная окраска голоса. Поддерживается только при выборе русского языка (ru-RU) и голосов jane или omazh.
     * @var string $emotion Эмоциональная окраска голоса. Поддерживается только при выборе русского языка (ru-RU) и голосов jane или omazh.
     * Допустимые значения:
     ...
     * @var int|float $speed Скорость (темп) синтезированной речи. Для премиум-голосов временно не поддерживается.
     ...
     * @var string $format Формат синтезируемого аудио.
     * Допустимые значения:
     ...
     * @var string|int|null $sampleRateHertz Частота дискретизации синтезируемого аудио.
     ...
     * @var string|null $folderId Идентификатор каталога, к которому у вас есть доступ. Требуется для авторизации с пользовательским аккаунтом (см. ресурс UserAccount ). Не используйте это поле, если вы делаете запрос от имени сервисного аккаунта.
